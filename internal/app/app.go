...
//
// The system prompt is composed dynamically from four sources:
//
//  1. The built-in Workshop prompt.
//  2. A contextual sentence describing the current working directory.
//  3. The catalog of available skills.
...
// Thread metadata is used for persistence (per-turn journal appends),
// while session metadata drives live status updates (PropertiesEvent).
//
...
func (c *compactCommand) Handler(ctx context.Context, _ loop.Emitter, cmd slash.Command) (slash.Result, error) {
	...
	// is load-bearing, mirroring thinkingCommand.writeLevel:
	//
...
//
// The handler reads from the bound session directly.
...
func setupSession(cfg *config) (*sessionSetup, error) {
	...
	// Slash command handlers. The thinking and analytics handlers
	// read from session metadata; the compact handler
...
// seedMetadata writes the static info-bar keys into the
// session's live metadata store. These power the TUI status zone
// (read via sess.AllMetadata()) and the slash handlers' session
// state; they are emitted on every SetMetadata call as a
// PropertiesEvent.
//
// The keys are thread_id, cwd (with home-prefix shortened to ~),
// git_branch (or "(not in git repo)"), tui.pid, and model (from
// defaultSpec.Name).
...
// a given configuration. It concatenates the built-in prompt with runtime
// context, the skills catalog, and repository instructions.
//
//  1. The built-in Workshop prompt.
//  2. A contextual sentence describing the current working directory.
...
// defaultPrompt is the baked-in Workshop system prompt.
